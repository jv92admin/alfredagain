...
class KitchenConfig(DomainConfig):
    ...
    def get_persona(self, subdomain: str, step_type: str) -> str:
        ...
        from alfred_kitchen.domain.personas import get_persona_for_subdomain
    ...
    def get_examples(
        ...
        from alfred_kitchen.domain.examples import get_contextual_examples
    ...
    def get_act_subdomain_header(self, subdomain: str, step_type: str) -> str:
        ...
        from alfred_kitchen.domain.personas import get_full_subdomain_content
    ...
    def get_table_format(self, table: str) -> dict[str, Any]:
        ...
        from alfred_kitchen.domain.formatters import TABLE_FORMAT_PROTOCOLS
    ...
    def get_empty_response(self, subdomain: str) -> str:
        ...
        from alfred_kitchen.domain.formatters import EMPTY_RESPONSES
    ...
    def get_subdomain_formatters(self) -> dict[str, Callable]:
        ...
        from alfred_kitchen.domain.formatters import (
    ...
    def format_entity_for_context(
        ...
        if entity_type == "recipe":
            from alfred_kitchen.domain.formatters import format_recipe_data
    ...
    def format_record_for_context(self, record: dict, table: str | None = None) -> str:
        ...
        from alfred_kitchen.domain.formatters import format_record_for_context
    ...
    def format_records_for_context(
        ...
        from alfred_kitchen.domain.formatters import format_records_for_context
    ...
    def format_records_for_reply(
        ...
        from alfred_kitchen.domain.formatters import format_records_for_reply
    ...
    def bypass_modes(self) -> dict[str, type]:
        ...
        from alfred_kitchen.domain.modes.cook import run_cook_session
        from alfred_kitchen.domain.modes.brainstorm import run_brainstorm
    ...
    def get_payload_compilers(self) -> list:
        ...
        from alfred_kitchen.domain.compilers import KITCHEN_COMPILERS
        return KITCHEN_COMPILERS

    def get_understand_prompt_content(self) -> str:
        """Kitchen-specific Understand prompt with all domain examples."""
        from alfred_kitchen.domain.prompts.understand_content import UNDERSTAND_PROMPT_CONTENT
        return UNDERSTAND_PROMPT_CONTENT

    def get_think_prompt_content(self) -> str:
        """Kitchen-specific Think system prompt with all domain examples."""
        from alfred_kitchen.domain.prompts.think_content import THINK_PROMPT_CONTENT
        return THINK_PROMPT_CONTENT

    def get_think_domain_context(self) -> str:
        """Kitchen domain context and philosophy for Think node."""
        from alfred_kitchen.domain.prompts.think_injections import THINK_DOMAIN_CONTEXT
        return THINK_DOMAIN_CONTEXT

    def get_think_planning_guide(self) -> str:
        """Kitchen planning guide with subdomains, linked tables, etc."""
        from alfred_kitchen.domain.prompts.think_injections import THINK_PLANNING_GUIDE
        return THINK_PLANNING_GUIDE

    def get_act_prompt_content(self, step_type: str) -> str:
        """Kitchen-specific full Act system prompt for each step type."""
        from alfred_kitchen.domain.prompts.act_content import get_act_prompt
        return get_act_prompt(step_type)

    def get_act_prompt_injection(self, step_type: str) -> str:
        """Kitchen-specific Act prompt guidance for each step type."""
        from alfred_kitchen.domain.prompts.act_injections import ACT_INJECTIONS
        return ACT_INJECTIONS.get(step_type, "")

    def get_reply_prompt_content(self) -> str:
        """Kitchen-specific Reply instructions with all domain examples."""
        from alfred_kitchen.domain.prompts.reply_content import REPLY_PROMPT_CONTENT
        return REPLY_PROMPT_CONTENT

    def get_reply_subdomain_guide(self) -> str:
        """Kitchen subdomain formatting guide for Reply node."""
        from alfred_kitchen.domain.prompts.reply_guide import REPLY_SUBDOMAIN_GUIDE
        return REPLY_SUBDOMAIN_GUIDE

    def get_router_prompt_injection(self) -> str:
        """Kitchen agent definitions for Router node."""
        from alfred_kitchen.domain.prompts.router_content import ROUTER_CONTENT
    ...
    def get_handoff_system_prompts(self) -> dict[str, str]:
        ...
        from alfred_kitchen.domain.handoff import HANDOFF_SYSTEM_PROMPTS
        return HANDOFF_SYSTEM_PROMPTS

    def get_handoff_result_model(self) -> type:
        """Get the HandoffResult Pydantic model."""
        from alfred_kitchen.domain.handoff import HandoffResult
    ...
    async def get_user_profile(self, user_id: str) -> str:
        """Return formatted kitchen user profile for prompt injection."""
    ...
    async def get_domain_snapshot(self, user_id: str) -> str:
        """Return formatted kitchen dashboard snapshot."""
        from alfred_kitchen.background.profile_builder import (
            format_dashboard_for_prompt,
            get_cached_dashboard,
        )
        dashboard = await get_cached_dashboard(user_id)
        return format_dashboard_for_prompt(dashboard)

    async def get_subdomain_guidance(self, user_id: str) -> dict[str, str]:
        """Return per-subdomain user guidance from kitchen profile."""
        from alfred_kitchen.background.profile_builder import get_cached_profile
        profile = await get_cached_profile(user_id)
        return profile.subdomain_guidance or {}
    ...
    def get_db_adapter(self):
        """Return Supabase client as database adapter."""
