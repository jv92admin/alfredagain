...
function CapabilityCard({ section }: { section: CapabilitySection }) {
  ...
  return (
    <motion.section
      ...
      className="bg-[var(--color-bg-elevated)] border border-[var(--color-border)] rounded-[var(--radius-lg)] p-4 sm:p-6 scroll-mt-6"
    >
      <h2 className="text-lg font-semibold text-[var(--color-text-primary)] mb-3">
        {section.title}
      </h2>

      <div className="text-sm text-[var(--color-text-secondary)] leading-relaxed whitespace-pre-line break-words">
      ...
      {section.tryPrompt && (
        <div className="mt-4 pt-4 border-t border-[var(--color-border)] flex items-start gap-3">
          <span className="text-sm text-[var(--color-text-muted)] italic min-w-0 break-words">
