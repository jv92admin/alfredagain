...
class TestCookModeInit:
    ...
    def test_init_fetches_recipe_and_sets_state(self):
        ...
        async def _test():
            with patch("alfred_kitchen.domain.modes.cook.db_read", new_callable=AsyncMock) as mock_read, \
...
class TestCookModeExit:
    ...
    def test_exit_with_save_action_injects_summary(self):
        ...
        async def _test():
            with patch("alfred_kitchen.domain.modes.cook.generate_session_handoff", new_callable=AsyncMock) as mock_fn:
...
class TestBrainstormModeInit:
    ...
    def test_init_loads_profile_and_dashboard(self):
        from alfred_kitchen.background.profile_builder import UserProfile, KitchenDashboard
        ...
        async def _test():
            with patch("alfred_kitchen.domain.modes.brainstorm.get_cached_profile", new_callable=AsyncMock) as mock_prof, \
                 patch("alfred_kitchen.domain.modes.brainstorm.get_cached_dashboard", new_callable=AsyncMock) as mock_dash, \
                 patch("alfred.llm.client.get_raw_async_client") as mock_client_fn, \
                 patch("alfred.llm.client.log_prompt"):
...
class TestBrainstormModeExit:
    ...
    def test_exit_with_save_injects_summary(self):
        ...
        async def _test():
            with patch("alfred_kitchen.domain.modes.brainstorm.generate_session_handoff", new_callable=AsyncMock) as mock_fn:
                mock_fn.return_value = mock_handoff
                from alfred_kitchen.domain.modes.brainstorm import run_brainstorm
                conversation = {
